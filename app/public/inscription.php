...
$courseExtras = [
    ...
    'Course Enfant'        => ['shortName'=>'3','unit'=>'km','color'=>'var(--sky)','total'=>30,'infos'=>[['🕚','Départ à 11h00'],['👦','De 8 à 11 ans'],['👨‍👧','Accompagnement adulte']],'urlParam'=>'3km'],
    ...
    try {
        ...
        foreach ($_POST as $key => $value) {
            if (strpos($key, 'meal_qty_') === 0 && intval($value) > 0) {
                ...
                $repasData[] = ['label' => $mealLabel, 'qty' => intval($value), 'amount' => $mealPrice * intval($value)];
            }
        }

        $totalCents = $courseAmount;
        foreach ($repasData as $r) $totalCents += $r['amount'];
...
<div class="page-header">
  ...
  <h1>Inscription</h1>
  <p><?= $showWidget ? 'Confirmez votre inscription' : 'Remplissez le formulaire' ?></p>
</div>
...
<div class="section">
  <div style="background:rgba(168,198,64,0.1);border:1px solid var(--lime);border-radius:4px;padding:2rem;text-align:center;max-width:600px;margin:0 auto;">
    ...
    <h2 style="font-family:'Bebas Neue',sans-serif;font-size:2rem;color:var(--lime);">Inscription confirmée !</h2>
    <p style="color:var(--sand);margin:1rem 0;">Un email de confirmation vous a été envoyé.</p>
...
<div class="section">
  <div style="background:rgba(196,68,10,0.15);border:1px solid var(--rust);border-radius:4px;padding:2rem;text-align:center;max-width:600px;margin:0 auto;">
    <div style="font-size:3rem;margin-bottom:1rem;">❌</div>
    <h2 style="font-family:'Bebas Neue',sans-serif;font-size:2rem;color:var(--rust);">Paiement annulé</h2>
    <p style="color:var(--sand);margin:1rem 0;">Le paiement n'a pas pu être finalisé.</p>
...
<section class="section">
  <div style="background:rgba(168,198,64,0.08);border:1px solid rgba(168,198,64,0.2);border-radius:4px;padding:1rem;margin-bottom:1.5rem;font-size:0.9rem;color:var(--sand);">
    ✅ <strong style="color:var(--cream)"><?= htmlspecialchars($inscData['prenom']??'') ?> <?= htmlspecialchars($inscData['nom']??'') ?></strong>
    — <strong style="color:var(--lime)"><?= htmlspecialchars($inscData['course']??'') ?></strong>
  </div>
  <?php
  $tierId = intval($inscData['tierId'] ?? 0);
  $params = http_build_query(['firstName' => $inscData['prenom']??'', 'lastName' => $inscData['nom']??'', 'email' => $inscData['email']??'']);
  if ($tierId) $params .= '&defaultTierCount['.$tierId.']=1';
  ?>
  <div style="border-radius:8px;overflow:hidden;border:1px solid rgba(255,255,255,0.1);">
    <iframe src="<?= $widgetUrl ?>?<?= $params ?>"
...
<?php foreach ($orderMap as $label):
    $course = null;
    foreach ($allCourses as $c) { if ($c['label'] === $label) { $course = $c; break; } }
    if (!$course) continue;
    $extras = $courseExtras[$label] ?? null;
    if (!$extras) continue;
    $color  = $extras['color'];
    $total  = $extras['total'];
...
<?php endforeach; ?>
    ...
    <div class="repas-grid">
      ...
      <div class="repas-row">
        <div class="repas-info"><strong><?= htmlspecialchars($meal['label']) ?></strong><span class="repas-prix"><?= number_format($meal['price'],2) ?> €</span></div>
        <div class="repas-qty">
    ...
    <button type="submit" class="submit-btn" disabled>Continuer →</button>
  </form>
</section>
<?php endif; ?>
...
var preselect=new URLSearchParams(window.location.search).get('course');
var preMap={'3km':'Course Enfant','7.5km':'Course 7.5km','15km':'Course 15km','test':'Course Test (payant)'};

document.querySelectorAll('.race-card').forEach(function(card){
  var tierId=parseInt(card.dataset.tierId),priceCents=parseInt(card.dataset.price),label=card.dataset.label;
  if(preselect&&preMap[preselect]===label) selectCard(card,tierId,priceCents,label);
...
document.querySelectorAll('.qty-btn').forEach(function(btn){
  btn.addEventListener('click',function(e){
    e.preventDefault();
    var id=btn.dataset.mealId,inp=document.querySelector('input[name="meal_qty_'+id+'"]'),v=parseInt(inp.value)||0;
    if(btn.classList.contains('qty-plus')) v++;
    else if(v>0) v--;
    inp.value=v; updatePrices();
  });
});
...
function updatePrices(){
  ...
  var mt=0;
  mealsData.forEach(function(m){var q=parseInt(document.querySelector('input[name="meal_qty_'+m.id+'"]').value)||0;mt+=q*m.price;});
  document.getElementById('meal-price').textContent=mt.toFixed(2)+' €';
  var t=(selectedPrice||0)+mt;
  document.getElementById('total-price').textContent=t.toFixed(2)+' €';
  var btn=document.querySelector('.submit-btn');
  if(btn) btn.textContent=t<=0&&selectedTierId?'Confirmer inscription gratuite →':'Continuer vers le paiement ('+t.toFixed(2)+' €) →';
}

function checkFormValidity(){
  var btn=document.querySelector('.submit-btn');
  if(!btn) return;
  btn.disabled=!(selectedTierId&&document.getElementById('prenom').value.trim()&&document.getElementById('nom').value.trim()&&document.getElementById('email').value.trim()&&document.getElementById('telephone').value.trim()&&document.getElementById('date_naissance').value&&document.getElementById('sexe').value);
}

document.querySelectorAll('input,select').forEach(function(el){el.addEventListener('input',checkFormValidity);el.addEventListener('change',checkFormValidity);});
var f=document.getElementById('inscription-form');
if(f) f.addEventListener('submit',function(){var b=document.querySelector('.submit-btn');b.textContent='⏳ Chargement...';b.disabled=true;});
updatePrices(); checkFormValidity();
</script>
</body>
</html>
